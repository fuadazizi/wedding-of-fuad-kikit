...
export function InvitationProvider({ children }) {
  ...
  useEffect(() => {
    ...
    if (guestParam) {
      try {
        ...
        console.error("Error parsing guest name:", error);
      }
    }
  ...
  // In static mode, always use the local config file
  const config = IS_STATIC_MODE ? staticConfig.data : apiConfig;

  return (
    <InvitationContext.Provider
      value={{ uid: invitationUid, config, isLoading: IS_STATIC_MODE ? false : isLoading, error: error?.message }}
    >
      {children}
    </InvitationContext.Provider>
  );
