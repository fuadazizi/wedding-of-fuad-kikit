...
export default function Wishes() {
  ...
  // Helper: persist submission to localStorage stats (developer access only)
  const recordStats = (entry) => {
    try {
      const raw = localStorage.getItem("sakeenah_stats");
      const stats = raw ? JSON.parse(raw) : { submissions: [] };
      stats.submissions.unshift(entry);
      localStorage.setItem("sakeenah_stats", JSON.stringify(stats));
    } catch {
  ...
  // Fetch wishes using React Query (skipped in static mode)
  const {
    data: apiWishes = [],
    isLoading,
    error,
  ...
  const wishes = IS_STATIC_MODE ? mockWishes : apiWishes;
  ...
  const handleSubmitWish = async (e) => {
    ...
    if (IS_STATIC_MODE) {
      setMockWishes((prev) => [newEntry, ...prev]);
